...
test('real prompt preserves every paragraph and keeps critical rules in startup context', () => {
  ...
  assert.match(bundle.core, /Import 3D sans prise de contrôle du PC/);
  assert.match(bundle.core, /Ne bascule jamais automatiquement vers la souris/);
  assert.match(bundle.core, /forge_get_model_import/);
  assert.match(bundle.core, /project-quality\.md/);
});
