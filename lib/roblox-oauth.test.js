...
test('développe le format compact réellement renvoyé par Roblox', () => {
  const compact = 'asset:read write game-pass:read write developer-product:read write openid profile';
  assert.deepEqual(canonicalizeRobloxScopes(compact), [
    'asset:read',
    'asset:write',
    'game-pass:read',
    'game-pass:write',
  ...
  assert.deepEqual(missingRobloxScopes(compact), []);
});
