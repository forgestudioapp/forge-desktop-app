...
function buildContextBundle(prompt) {
  ...
  const kit = ['ForgeUI.lua', 'README.md'].map(name => ({ name,
    content: fs.readFileSync(path.join(__dirname, 'gui-kit', name), 'utf8').replace(/\r\n/g, '\n') }));
  const version = crypto.createHash('sha256').update(source + JSON.stringify(kit)).digest('hex').slice(0, 16);
  const resources = kit.map(file => ({ path: `.forge-context/${version}/gui-kit/${file.name}`, content: file.content }));
  const guides = [];
  ...
  headings.forEach((heading, i) => {
    ...
    if (!guide) core.push(section);
      ...
      (Number(heading[1]) === 11 ? `\nPour une base réutilisable de boutons et fenêtres Luau, consulte \`.forge-context/${version}/gui-kit/README.md\`. Adapte le thème au jeu et conserve le framework existant.\n` : '') });
  });
  core.splice(1, 0, [
...
function syncForgeContext(projectPath, agentType, prompt) {
  ...
  try {
    for (const guide of [...(bundle.resources || []), ...bundle.guides]) {
