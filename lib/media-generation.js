...
function buildCodexMediaInstructions({ kind, label, prompt, count, sourceImage, outputPaths, referenceImages = [] }) {
  ...
  return [
    ...
    sourceImage
      ? `La première image jointe est la source à modifier (${sourceImage}). Respecte sa composition sauf pour les changements demandés.`
      : `Il s'agit d'une création originale sans image source.`,
    ...(referenceImages.length ? [
      `Les ${referenceImages.length} images suivantes sont des références supplémentaires : utilise-les selon la demande sans les confondre avec la source à modifier.`,
      ...referenceImages.map((file, index) => `Référence ${index + 1} : ${file}`),
    ] : []),
    formatInstruction,
