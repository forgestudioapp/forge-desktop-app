...
function viewer() {
  ...
  const noop = () => {};
  const mockEl = { style: {}, appendChild: noop, remove: noop, click: noop, setAttribute: noop, addEventListener: noop, removeEventListener: noop };
  const document = { hidden: false, addEventListener: (name, callback) => listeners.set(name, callback), removeEventListener: name => listeners.delete(name), createElement: () => ({ ...mockEl, appendChild: noop }), activeElement: null };
  ...
  const canvas = { width: 400, height: 300, isConnected: true, getBoundingClientRect: () => ({ width: 400, height: 300 }), parentElement: { style: {} } };
