...
test('autorise le transfert explicite de la place depuis le projet en conflit', () => {
  const projects = [{ name: 'Ancien', path: 'C:\\ForgeProjects\\Ancien', linkedStudio: { placeId: '123' } }];
  const result = evaluateProjectPlaceLink(projects, 'C:\\ForgeProjects\\Nouveau', place, '2026-09-25T00:00:00.000Z', {
    allowReassign: true,
    reassignFromProjectPath: projects[0].path,
  });
  assert.equal(result.success, true);
  assert.equal(result.reassignedFromProjectPath, projects[0].path);
  assert.equal(result.link.placeId, '123');
});

test('refuse un transfert qui ne designe pas exactement le projet en conflit', () => {
  const projects = [{ name: 'Ancien', path: 'C:\\ForgeProjects\\Ancien', linkedStudio: { placeId: '123' } }];
  const result = evaluateProjectPlaceLink(projects, 'C:\\ForgeProjects\\Nouveau', place, undefined, {
    allowReassign: true,
    reassignFromProjectPath: 'C:\\ForgeProjects\\Autre',
  });
  assert.equal(result.code, 'place-linked-elsewhere');
});
