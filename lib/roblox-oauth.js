...
function missingRobloxScopes(scopeValue) {
  ...
  const granted = new Set(rawScopes.map(scope => String(scope).trim()).filter(Boolean));
...
function normalizeRobloxTokenData(tokenData) {
  ...
  const returnedScopes = Array.isArray(data.scope)
    ? data.scope.map(scope => String(scope).trim()).filter(Boolean)
    : String(data.scope || '').split(/[\s,]+/).filter(Boolean);
